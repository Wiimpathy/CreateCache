...
u8 *fsop_ReadFile(const char *path, u32 *size)
{
	*(size) = 0;
	...
	{
		//printf("filesize > 0\n");
		//sprintf(buferror, "filesize > 0\n");
		mem = (u8*)malloc(filesize);
		if(mem != NULL)
		{
			//sprintf(buferror, "fsop_ReadFile : malloc failed\n");
			//printf("mem != NULL\n");
...
TexErr STexture::fromJPG(TexData &dest, const u8 *buffer, const u32 buffer_size, u8 f, u32 minMipSize, u32 maxMipSize)
{
	Cleanup(dest);
	dest.data = NULL;
	printf("JPG decoding not implemented!\n");
	sprintf(buferror, "JPG decoding not implemented!\n");
	return TE_ERROR;
#if 0
	Cleanup(dest);
...
TexErr STexture::fromPNG(TexData &dest, const u8 *buffer, u8 f, u32 minMipSize, u32 maxMipSize, bool reduce_alpha)
{
	...
	if (maxLODTmp > 0)
	{
			...
			&& imgProp.imgWidth <= 640 && imgProp.imgHeight <= 480)
		{
			...
			dest.dataSize = GX_GetTexBufferSize(dest.width, dest.height, dest.format, GX_FALSE, 0);
			dest.data = (u8*)malloc(dest.dataSize);
...
u8 *STexture::_genMipMaps(u8 *&src, u32 width, u32 height, u8 maxLOD, u32 lod0Width, u32 lod0Height)
{
	...
	u8 *dstData = (u8*)malloc(bufSize);

	if(dstData == NULL)
	{
		printf("_genMipMaps() dstdata NULL");
		sprintf(buferror, "_genMipMaps() dstdata NULL\n");
		return NULL;
	}
